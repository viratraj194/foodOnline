...
from accounts.views import check_role_for_vendor
from orders.models import Order, OrderedFood
from vendor.forms import VendorForms,OpeningHourForm
...
# category.objects.filter (vendor—get vendor(request))
...
def add_opening_hours(request):
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
            ...
            # print(day,from_hour,to_hour,is_close)
            try:
                hour = OpeningHours.objects.create(vendor=get_vendor(request), day=day, from_hour=from_hour, to_hour=to_hour, is_close=is_close)
                if hour:
                    day = OpeningHours.objects.get(id=hour.id)
                    if day.is_close:
                        response = {'status': 'success', 'id': hour.id, 'day': day.get_day_display(), 'is_close': 'Close'}
                    else:
